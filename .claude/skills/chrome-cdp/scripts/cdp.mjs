...
const TMP_DIR = process.platform === 'win32' ? 'D:/tmp' : '/tmp';
const SOCK_PREFIX = `${TMP_DIR}/cdp-`;
const PAGES_CACHE = `${TMP_DIR}/cdp-pages.json`;

function sockPath(targetId) { return `${SOCK_PREFIX}${targetId}.sock`; }

function getWsUrl() {
  // Windows: When Chrome is started with --remote-debugging-port=9222,
  // we can directly use the default endpoint
  if (process.platform === 'win32') {
    return 'ws://127.0.0.1:9222/devtools/browser';
  }
  // macOS: Read from DevToolsActivePort
